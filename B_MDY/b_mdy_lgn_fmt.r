...
server <- shinyServer(function(input, output) {
  ...
  df_products_upload <- reactive({
    ...
    Address	<- df$char5
    Bill_Code	<- df$char17
    Due_Date <- ifelse(nchar(df$char10) == 10,
                       Due_Date <- df$char10,
                       Due_Date <- format(as.Date(as.numeric(df$char10),"1899-12-30"),"%d-%m-%Y"))
    Units	<- as.numeric(df$char8)
    Electrical_Fees	<- as.numeric(df$char26)+as.numeric(df$char27)+as.numeric(df$char28)+as.numeric(df$char29)+as.numeric(df$char30)+as.numeric(df$char31)+as.numeric(df$char32)
    ...
    Debt	<- as.numeric(df$char32)	
    Fine	<- as.numeric(df$char32)
  ...
  output$downloadData <- downloadHandler(
    filename = function() {paste(input$billCode,"_20240914110859562.xlsx", sep="")},
