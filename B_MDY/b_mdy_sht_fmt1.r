...
ui <- shinyUI(fluidPage(
  ...
  # App title ----
  titlePanel("IO-BILL MDY SERVICE"),
...
server <- shinyServer(function(input, output) {
  ...
  df_products_upload <- reactive({
    ...
    Address	<- df$char5
    Bill_Code	<- df$char6
    Due_Date <- ifelse(nchar(df$char7) == 10,
                       Due_Date <- df$char7,
                       Due_Date <- format(as.Date(as.numeric(df$char7),"1899-12-30"),"%d-%m-%Y"))
    Units	<- as.numeric(df$char8)
    Electrical_Fees	<- as.numeric(df$char9)
    Service_Charges	<- as.numeric(df$char10)
    Horse_Power_Fees	<- as.numeric(df$char11)
    Discount	<- 0.00
    Last_Balance <- 0.00
    Total	<- as.numeric(df$char12)
    Debt	<- as.numeric(df$char13)	
    Fine	<- as.numeric(df$char14)
  ...
  output$downloadData <- downloadHandler(
    filename = function() {paste(input$billCode,"_20240813114239574.xlsx", sep="")},
