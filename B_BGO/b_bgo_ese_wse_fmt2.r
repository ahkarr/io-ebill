...
server <- shinyServer(function(input, output) {
  ...
  df_products_upload <- reactive({
    ...
    Due_Date <- ifelse(nchar(df$char11) == 10,
                       Due_Date <- df$char11,
                       Due_Date <- format(as.Date(as.numeric(df$char11),"1899-12-30"),"%d-%m-%Y"))
    Units	<- as.numeric(df$char9)
    Electrical_Fees	<- as.numeric(df$char10)	
    Service_Charges	<- 0.00
    Horse_Power_Fees	<- 0.00
    Discount	<- 0.00
    Last_Balance <-  0.00
    Total	<- as.numeric(df$char10)
    Debt	<- 0.00
    Fine	<- 0.00
    Grand_Total <- as.numeric(df$char10)
